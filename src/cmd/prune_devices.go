...
// whatsmeow_device store that no device slot references any more. It reuses the exact
// same DB containers and chat-storage registry the service builds in initApp (wired by
// cobra.OnInitialize), but never starts the REST server, never auto-connects a device,
// and never dials WhatsApp. Dry-run by default; deletes only with --apply.
...
DeviceJID of any slot in the devices table. Only the PERSISTED AD JID is consulted: this
is a separate process and cannot see a running service's in-memory registry.

SAFETY GATES
  1. Un-backfilled slots: if any slot still has an empty DeviceJID (legacy / not yet
...
func pruneDevices(_ *cobra.Command, _ []string) {
	...
	// DeviceJID) instead contribute their bare number to the protected-numbers set
	// (safety gate 1).
	...
	for _, rec := range records {
		...
		if adJID := strings.TrimSpace(rec.DeviceJID); adJID != "" {
