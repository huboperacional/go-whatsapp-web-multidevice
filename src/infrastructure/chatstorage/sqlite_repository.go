...
func (r *SQLiteRepository) SaveDeviceRecord(record *domainChatStorage.DeviceRecord) error {
	...
	// Try update first, then insert if no rows affected (cross-db compatible)
	result, err := r.db.Exec(`
		UPDATE devices SET display_name = ?, jid = ?, device_jid = ?, updated_at = ?
		WHERE device_id = ?
	`, record.DisplayName, record.JID, record.DeviceJID, record.UpdatedAt, record.DeviceID)
	if err != nil {
		return err
	}
	...
	if rowsAffected == 0 {
		_, err = r.db.Exec(`
			INSERT INTO devices (device_id, display_name, jid, device_jid, created_at, updated_at)
			VALUES (?, ?, ?, ?, ?, ?)
		`, record.DeviceID, record.DisplayName, record.JID, record.DeviceJID, record.CreatedAt, record.UpdatedAt)
	}
	return err
}
...
func (r *SQLiteRepository) ListDeviceRecords() ([]*domainChatStorage.DeviceRecord, error) {
	rows, err := r.db.Query(`
		SELECT device_id, display_name, jid, COALESCE(device_jid, ''), created_at, updated_at
		FROM devices
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rec.DeviceID, &rec.DisplayName, &rec.JID, &rec.DeviceJID, &rec.CreatedAt, &rec.UpdatedAt); err != nil {
			return nil, err
		}
...
func (r *SQLiteRepository) GetDeviceRecord(deviceID string) (*domainChatStorage.DeviceRecord, error) {
	...
	err := r.db.QueryRow(`
		...
		WHERE device_id = ?
		LIMIT 1
	`, deviceID).Scan(&rec.DeviceID, &rec.DisplayName, &rec.JID, &rec.DeviceJID, &rec.CreatedAt, &rec.UpdatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
...
func (r *SQLiteRepository) GetDeviceRecordByJID(jid string) (*domainChatStorage.DeviceRecord, error) {
	...
	rec := &domainChatStorage.DeviceRecord{}
	err := r.db.QueryRow(`
		SELECT device_id, display_name, jid, COALESCE(device_jid, ''), webhook_url, COALESCE(webhook_secret, ''), COALESCE(webhook_events, ''), COALESCE(webhook_insecure_skip_verify, FALSE), created_at, updated_at
		FROM devices
		WHERE jid = ?
		LIMIT 1
	`, jid).Scan(
...
func (r *SQLiteRepository) getMigrations() []string {
	return []string{
		...
		// Migration 35: Store the full AD (device-scoped) JID for a slot, e.g.
		// 5567...:32@s.whatsapp.net. The `jid` column keeps the NonAD form (the bare
		// number) because it is the chatstorage partition key. Empty string means
		// "unknown / legacy row" -- it is backfilled on the device's next connect.
		`ALTER TABLE devices ADD COLUMN device_jid TEXT DEFAULT ''`,
	}
}
